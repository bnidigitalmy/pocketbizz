...
-   **Smart Supplier Order Hub**: Advanced purchase order management system with draft→sent→received→cancelled workflow. Features include editable shopping cart with quantity adjustments, manual item addition, multi-supplier selection, **professional Malaysian-format PDF purchase orders with SST compliance**, **email sending with PDF attachments via Resend**, WhatsApp PO sharing to suppliers, automatic stock replenishment upon receipt, auto-expense creation linked to purchases, PO history tracking, **draft PO editing** (supplier info, notes), and **template management for recurring orders** (save PO as template, create from template). Replaces basic shopping cart with full procurement lifecycle management.
...
-   **Loyalty Points Program**: Simple points-based reward system with customer tracking via phone number, automatic point earning (RM1 = 1 point), point redemption as discounts (100 points = RM10), transaction history, and seamless POS integration. Designed for retail businesses to increase customer retention and repeat purchases.
-   **Broadcast Messaging System**: Multi-channel customer engagement platform supporting WhatsApp, SMS, and Email broadcasts. Features intelligent customer segmentation (all customers, high points 500+, recent buyers within 30 days), 6 pre-built Bahasa Melayu message templates for common scenarios (promotions, new products, vouchers, festive greetings), campaign history tracking, recipient preview before sending, and integration-ready architecture for Twilio (WhatsApp/SMS) and Resend (Email). Full Bahasa Melayu interface under "Jualan & Operasi" section.
...
-   **ToyyibPay**
-   **Stripe**
-   **Resend** (Email API for PO delivery)