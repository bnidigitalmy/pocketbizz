...
**Sidebar Navigation**: Accordion-style collapsible menu groups organize features into 6 logical categories (Overview, Pengurusan Stok, Jualan & Operasi, Kewangan, Ejen Jualan, Sistem). Only one section expands at a time, with automatic expansion of the section containing the current route. Includes logout button in footer for easy session management.
