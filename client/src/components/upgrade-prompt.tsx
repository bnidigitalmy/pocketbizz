...
export function UpgradePrompt() {
  ...
  useEffect(() => {
    ...
    const isOnActiveTrial = user.isOnTrial === 1 && user.trialEndsAt && new Date(user.trialEndsAt) > new Date();
    
    // Check if trial has expired
    const trialExpired = user.isOnTrial === 1 && user.trialEndsAt && new Date(user.trialEndsAt) < new Date();
    ...
    if (isOnActiveTrial) {
      ...
      // Not on trial, no active subscription (expired paid user)
      setShowPrompt(true);
    }
